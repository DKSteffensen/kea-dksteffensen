...
echo ($sUsers); // shows scaping \" but JS works
	...
	$sQuery = $oDb->prepare("UPDATE users SET name='$newUserName' WHERE id = $userID");
	$sQuery->execute();
	$iRowsUpdated = $oDb->changes();
	echo $iRowsUpdated;
}
else if($action == "updateAllUsers"){
	$oldNames = $_POST['oldname'];
	$newUserName = $_POST['newusername'];

	$oDb = new SQLite3('db/database.sql3');

	$sQuery = $oDb->prepare("UPDATE users SET name='$newUserName' WHERE name = '$oldNames'");
	$sQuery->execute();
	$iRowsUpdated = $oDb->changes();
	echo $iRowsUpdated;	
}
	...
	$sQuery = $oDb->prepare("INSERT INTO users(id, name) VALUES (null, '$newName')");
	$sQuery->execute();
	$iRowChanged = $oDb->changes();
	$iLastUpdateID = $oDb->lastInsertRowid();
